...
#Created by dobkeratops  #http://github.com/dobkeratops/convnet_stuff
#
# Created by dobkeratops  
# http://github.com/dobkeratops/convnet_stuff
# Edits and testing by Twenkid #4-10-2022
# NoisedImageDataset(TransformImageDataset) ... and calls super.__init__( ... ) in order to have io_channels etc.
# Edits on the directory separator (Windows) but it was not the problem 
# make_dataloader
# load_image_as_tensor
# forcing the device to CPU in order to avoid crashes of the PC (low GPU memory, LOL)
...
class TransformImageDataset(Dataset):
	...
	def io_channels(self):
		...
		print("io_channels: first_in.shape[0],first_out.shape[0]",first_in.shape[0],first_out.shape[0])
...
class NoisedImageDataset(TransformImageDataset): #(Dataset):
	def __init__(self,dirname,max=1000000000,noise=0.33): 
		...
		super().__init__(dirname,max)
		print(dir(self))
		print("image_pairs",self.image_io_pairs) #TWENKID
...
#	../training_images/ --> ../training_images TWENKID
...
#"../training_images/","current_model/",None
def main(argv):
	inputdir,outputdir,pretrained= "..\\training_images","current_model/",None,
	learning_rate = 0.1
